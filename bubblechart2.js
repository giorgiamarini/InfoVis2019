...
d3.json("data.json").then(function(data){
    ...
    for (let i = 0; i < 20; i++){
        let circle = svg.append("circle")
            .attr("id", "#b"+i)
            .attr("cx", circles[i.toString()].x)
            .attr("cy", circles[i.toString()].y)
            .attr("r", circles[i.toString()].r)
        ...
        .on("click", function(){
            if (clickNumber == 0){
                ...
                svgCircles.forEach(function(element, index){
                    console.log(id[v] + "   #b" + index)
                    d3.select("#b" + index)
                        .transition().duration(2000)
                        .attr("r", circles[v.toString()].y * 0.10)
                        .attr("cx", circles[v.toString()].r * 10)
                        .attr("cy", circles[v.toString()].x * 0.60);
                    v++;
